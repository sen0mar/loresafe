...
export const ClubMilestoneBuilderPanel = ({ club }: { club: Club }) => {
  ...
  const submitTemplate = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!parseResult.success) {
      ...
      setTemplateErrors({
        ...
        safeTitleRows: getTemplateSafeTitleRowErrors(parseResult.error.issues)
      });
      return;
    }
  ...
  return (
    <ClubSettingsDisclosure
      contentId={builderContentId}
      description="Add one checkpoint or generate an empty timeline from a template."
      icon={ListPlus}
        ...
        <form className="mt-4 grid gap-3" onSubmit={submitMilestone} noValidate>
          ...
          <label className="flex items-start gap-3 rounded-lg border border-default bg-surface p-3 text-sm text-muted">
            ...
            <span className="grid gap-1">
              ...
              <span>
                Readers will see the spoiler free title until the spoiler title
                is safe to show, once the needed milestone has been reached.
              </span>
            </span>
          </label>
        ...
        <form className="mt-4 grid gap-4" onSubmit={submitTemplate} noValidate>
          ...
          <div className="rounded-lg border border-default bg-surface p-3">
            ...
            <p className="mt-2 text-sm leading-6 text-muted">
              These are spoiler free preview titles. Add full spoiler titles
              later in the Timeline tab.
            </p>
            ...
            {visibleTemplateTitles.length > 0 ? (
              <ol className="mt-3 grid max-h-72 gap-2 overflow-y-auto pr-1">
                {visibleTemplateTitles.map((title, index) => {
                  ...
                  const titleError = templateErrors.safeTitleRows?.[index];
...
const FormError = ({ message }: { message: string }) => (
  <div className="rounded-lg border border-default bg-surface p-3" role="alert">
